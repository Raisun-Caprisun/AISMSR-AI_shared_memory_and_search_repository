...
' Version: 3.5 - Definitive Obstacle and Wall Handling
' Description: This version corrects the logic in the 'PreloadObstacles' function
'              to correctly identify "Walls" as fixed obstacles, ensuring the
'              placement algorithm works with a complete and accurate map.
'---------------------------------------------------------------------------------------
Option Explicit
...
Public Sub RunFirstCycle_Placement()
    Debug.Print "Running First Cycle Placement Optimization..."
    RunPlacementOptimization useNewWidth:=False
...
Public Sub RunSecondCycle_Placement()
    Debug.Print "Running Second Cycle Placement Optimization..."
    RunPlacementOptimization useNewWidth:=True
...
Private Sub RunPlacementOptimization(ByVal useNewWidth As Boolean)
    Dim ws As Worksheet
    ...
    If inboundX = -1 Then MsgBox "'Inbound' area not found.", vbCritical: GoTo Cleanup
    ...
    If areas.count = 0 Then MsgBox "No 'Areas' found to optimize.", vbExclamation: GoTo Cleanup
...
Private Sub PreloadObstacles(ByVal ws As Worksheet, ByVal lastRow As Long, ByVal cols As Object, ByRef placed As Collection)
    ...
    For r = 2 To lastRow
        ...
        ' This condition now correctly identifies Walls as obstacles.
        Select Case layerName
            Case "area", "areas", "zone", "zones", "walk", "inbound"
                ' Do nothing for these layers
            Case Else
                ' Treat everything else (including "Walls", "Office", etc.) as a fixed obstacle
